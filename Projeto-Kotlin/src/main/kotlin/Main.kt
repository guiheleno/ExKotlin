...
fun doar(nome: String, cpf: String, endereco: String, email: String, idade: Int){
    var doador = Doador(nome, cpf, endereco, email, idade)
    while(true){
        println("==============SISTEMA DOADOR=====================")
        println("{1} - DOAR")
        println("{2} - ALTERAR")
        println("{3} - MEU HISTÓRICO DOAÇÕES")
        println("{4} - LISTAR ESTOQUE")
        println("{5} - SAIR")
        val opc = readln().toInt()
        when(opc){
            1 -> {
                println("DIGITE UM ITEM PARA SER DOADO:")
                var item = readln()
                doador.doacao(item)
            }
            2-> {
            ...
            3 -> doador.exibirLista()
            4 -> doador.exibirHistoticoD()
            5 -> break
